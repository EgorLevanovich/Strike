...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_Player : MMFeedbacks
	{
		...
		public override void Initialization(bool forceInitIfPlaying = false)
		{
			...
			int count = FeedbacksList.Count;
			var owner = _useGlobalOwner ? GlobalMMFPlayer.Instance : this;
			for (int i = 0; i < count; i++)
			{
				if (FeedbacksList[i] != null)
				{
					FeedbacksList[i].Initialization(owner, i);
				}
			}
		...
		protected override void PlayFeedbacksInternal(Vector3 position, float feedbacksIntensity, bool forceRevert = false)
		{
			...
			if (InitialDelay > 0f)
			{
				StopInitialDelayCoroutine();

				_initialDelayCoroutine = HandleInitialDelayCo(position, feedbacksIntensity, forceRevert);
				StartCoroutine(_initialDelayCoroutine);
		...
		private IEnumerator _initialDelayCoroutine;

		public void StopInitialDelayCoroutine()
		{
			if (_initialDelayCoroutine != null)
			{
				StopCoroutine(_initialDelayCoroutine);

...
{	
	...
	public abstract class MMObjectPooler : MonoBehaviour
	{
		...
		private readonly string[] _cachedNumbers = new string[1000];
		...
		public string GetCachedNumber(float value)
		{
			int index = (int)value;
			return (uint)index < (uint)_cachedNumbers.Length
				? _cachedNumbers[index] 
				: value.ToString();
		}
        
		public void Dispose()
		{
			foreach (var item in _objectPool.PooledGameObjects.Where(item => item != null))
			{
			...
			{
				foreach (var item in pool.PooledGameObjects.Where(item => item != null))
				{
					item.gameObject.SetActive(false);
				}
				pool.PooledGameObjects?.Clear();
			}
		}
	}
}